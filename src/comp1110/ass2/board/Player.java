...
public class Player {
    ...
    public static int playerScore(String stateString, int playId) {
        int[] scores = calculateScores(stateString);
        int score = scores[playId];
        return score;
    }
    ...
    /**
     * get which player is on phase
     * @param gameStates State string provided
     * @return the playId of the player who is playing
     */
    public static int getCurrentPlayer (String gameStates){
        String[] states = gameStates.split("; ");
        int playerId = 0;
        for (String state : states){
            if (state.startsWith("c")){
                playerId = Integer.parseInt(String.valueOf(state.charAt(2)));
                break;
            }
        }
        return playerId;
